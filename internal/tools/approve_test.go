...
func TestApprovalByRefusesTheAuthorFirst(t *testing.T) {
	d := decision{repo: teamfiles.Repo{Owner: "giantswarm", Name: "github", Ref: "main"}, number: 6119, team: testTeam, author: "alice"}
	...
	if !a.Member || a.Author != "alice" || a.Team != testTeam || a.PullRequest != 6119 || a.Login != "carol" {
...
func TestAskNamesWhoDecides(t *testing.T) {
	if got, want := decides(testTeam, "alice"), " A member of team-bumblebee other than alice approves."; got != want {
