...
| reviews.channels | object | `{}` | Slack channel IDs by the channel names a team's policy file carries: `slackChannel` (asks with an Approve button) and `standupChannel` (notices). The gateway takes IDs, so every channel a policy file names needs an entry (`team-x: C…`, `standup-x: C…`) unless the file carries the ID itself. |
| reviews.debugChannel | string | `""` | A channel that receives every ask and notice instead of the channel the policy file names, the text closing with that channel (`… (for #team-x)`): a test round of the flows without disturbing the teams. A name `channels` resolves or a Slack channel ID; empty delivers to the policy file's channel. |
