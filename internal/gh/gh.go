...
// installation, or as a token in development, with the REST budget it drew
...
// TokenReader is a Reader on a personal or fine-grained token — development
// only: it draws from the token owner's shared budget.
func TokenReader(apiURL, token string) (*Reader, error) {
	token = strings.TrimSpace(token)
	if token == "" {
		return nil, fmt.Errorf("github: a token is required")
	}
	counter := &counter{base: http.DefaultTransport}
	tr := &bearerTransport{token: token, base: counter}
	return newReader(apiURL, tr, counter, "token", func(context.Context) (string, error) { return token, nil })
}
...
// Name says which identity reads: "app installation <id>" or "token".
