...
func (o *fakeOrg) handle(w http.ResponseWriter, r *http.Request) {
	...
	case contains(req.Query, "repository(owner: $org, name: $name)"):
		...
		if n := o.node(name); n != nil {
			...
			data["repository"] = nil
			errs = append(errs, map[string]any{"message": fmt.Sprintf("Could not resolve to a Repository with the name '%s/%s'.", org, name), kType: "NOT_FOUND", kPath: []string{"repository"}})
		}
	default:
