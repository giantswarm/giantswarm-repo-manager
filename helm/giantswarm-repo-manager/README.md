...
| inventory.sweep.interval | string | `"24h"` | Full sweep over the org every interval (a Go duration); `"0"` turns the schedule off. The first sweep runs at start when the last one is older than the interval; an unfinished sweep younger than the interval, one a restarted pod left, is taken up at start and keeps the records it wrote. |
...
| inventory.sweep.restBudgetFloor | int | `500` | Pause a sweep's engine checks until the REST budget resets when its remaining requests fall below this, instead of failing the remaining checks with GitHub's rate-limit refusal; 0 never pauses. |
...
| inventory.engine.devctlAppID | int | `0` | Numeric id of the devctl GitHub App (the App's settings page; not the client id), the reconciler's bypass actor on the ruleset `devctl: default branch`. 0, the default: the engine's read-mode protection step compares the ruleset's rules and leaves its bypass list uncompared, the summary saying `bypass actors not compared`. GitHub shows a ruleset's bypass actors to identities that administer the repository, and the inventory App reads: with the id the step would compare an empty list against the reconciler's and report every aligned repository as drifted. Set it only for a read identity that sees the bypass actors. Passed as `DEVCTL_APP_ID`. |
| inventory.releases.interval | string | `"5m"` | Follow the latest release of every declared repository from its tag to the end of the tag's own CircleCI pipeline, reading every interval (a Go duration): one GraphQL query over the most recently pushed repositories per pass, and CircleCI reads for the releases still running. A red pipeline, or none within `grace`, is one notice to the owning team's `notices` channel, told once; a rerun that goes green is silent. `"0"` turns the watch off. Public repositories are read without a token; a private one needs `circleci.existingSecret`, else its release reads `unchecked`. |
| inventory.releases.grace | string | `"10m"` | How long after its creation a release may have no CircleCI pipeline before it is a missed build (`missed-tag-build`, told once). |
| inventory.reconciler.pollInterval | string | `"5m"` | Read the reconciler workflow's completed runs from GitHub as the inventory App every interval (a Go duration) and store each run's `reconcile-<repository>` artifact as the repository's `setup.lastRun`; every 30 s while an Align now (`align_repository`) is pending. `"0"` turns the poller off. Nothing reaches the server from the workflow: the App needs Actions read on `teamFiles.repository`. |
| inventory.reconciler.workflow | string | `"reconcile-repositories.yaml"` | The reconciler's workflow file in `teamFiles.repository`: the one `align_repository` dispatches and the poller reads the runs of. |
| inventory.renovate.activeDays | int | `180` | Days within which a Renovate pull request or commit counts as Renovate activity — `list_repositories`' Renovate state `active`, else `inactive`. Passed as `RENOVATE_ACTIVE_DAYS`. |
| teamFiles.repository | string | `"giantswarm/github"` | The repository that holds the team files (`repositories/<team>.yaml`, the desired state) and the teams' channel files (`teams/<team>.yaml`); every write is a pull request against it, opened as the caller. |
| teamFiles.ref | string | `"main"` | The branch the files are read from and pull requests target. |
| circleci.existingSecret | string | `""` | Existing Secret with a CircleCI API token under `token`, for the release watch to read the tag pipelines of private repositories (a personal token of a read-only identity: CircleCI has no scoped tokens). Empty reads anonymously, which CircleCI answers for public projects alone; a private repository's release then reads `unchecked` and the team hears nothing about it. `watch_repository` decides a new repository's released phase from the tag pipeline wherever the release watch reads it, from the commit statuses elsewhere. |
| reviews.gatewayURL | string | `""` | klaus-gateway's base URL as reached from the pod (for example `http://klaus-gateway.agent-platform.svc:8080`): lifecycle and transfer asks go to `POST /reviews` into the team's `asks` channel, notices (the giving team's transfer notice, what a reconciler run tells the team) to `POST /notices` into its `notices` channel (both from the team's `teams/<team>.yaml`), authenticated with this pod's projected ServiceAccount token for `audience`. Empty leaves the asks undelivered; the tools say so and approving on GitHub stays equivalent. `get_info` reports it as `reviews.url` beside `reviews.configured`. |
| reviews.audience | string | `"klaus-gateway"` | The audience of the projected token — the gateway's `reviews.audience`; this ServiceAccount must be in its `reviews.allowedCallers`. `get_info` reports it as `reviews.audience`. |
| reviews.channels | object | `{}` | Unread: the channels are each team's `teams/<team>.yaml` in the team-files repository, their IDs and names. Still accepted, and ignored, so a values file that sets it keeps validating until it drops the key. |
| reviews.debugChannel | string | `""` | A Slack channel ID that receives every ask and notice instead of the team's channel, the text closing with that channel's name (`… (for #team-x)`): a test round of the flows without disturbing the teams. Empty delivers to the channel the team's `teams/<team>.yaml` names. |
...
| muster.mcpServer.timeout | int | `180` | Seconds muster waits for one call before it cancels it (the CR's `spec.timeout`; muster's own default is 30, the CRD allows 1–300). 180 because `create_repository` in `mode: commit` does its writes in one call — create the repository, fetch and render the template, push the scaffold, open the pull request — `watch_repository` blocks up to its `timeout` of at most 150 s, and `align_repository` and `set_lifecycle` with the ask come close to muster's default as well. |
