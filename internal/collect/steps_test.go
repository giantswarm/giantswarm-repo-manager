...
func TestFillClientlessSteps(t *testing.T) {
	...
	cases := []struct {
		...
		{"circleci: a repair that followed and left the project deaf reports it", engine(skipCircle), record(deaf, nil, run(reconcile.ModeRepair, reconcile.StepResult{Step: reconcile.StepCircleCI, Verdict: reconcile.VerdictRepaired, Changes: []string{"follow giantswarm/x"}, Findings: []reconcile.Finding{webhookMissing}})), reconcile.StepCircleCI,
