...
| inventory.engine.devctlAppID | int | `5025978` | Numeric id of the devctl GitHub App (the App's settings page; not the client id), the reconciler's bypass actor on the ruleset `devctl: default branch`: with it the engine's read-mode protection step compares the ruleset's bypass list too (the devctl App, the owning team and the repository admins for pull requests, as the reconciler writes them) and plans a repository still on classic protection as the reconciler would; the check writes nothing. The id is public. 0 compares the rules alone, the summary saying `bypass actors not compared`. Passed as `DEVCTL_APP_ID`. |
